/*Задание 2
В HTML есть пустой список ul#ingredients.

<ul id="ingredients"></ul>
В JS есть массив строк.

const ingredients = [
  'Картошка',
  'Грибы',
  'Чеснок',
  'Помидоры',
  'Зелень',
  'Приправы',
];
Напиши скрипт, который для каждого элемента массива ingredients создаст отдельный li, после чего вставит все li за одну 
операцию в список ul.ingredients. Для создания DOM-узлов используй document.createElement().
 */


console.log('------------------task2 start-------------------');

const ingredients = [
  'Картошка',
  'Грибы',
  'Чеснок',
  'Помидоры',
  'Зелень',
  'Приправы',
];

const ingredientsList = ingredients.map(ingredient => {
  const ingredientFormatted = document.createElement("li");
  ingredientFormatted.textContent = ingredient;
  return ingredientFormatted;
});

const par = document.querySelector("#ingredients");

par.append(...ingredientsList);

console.log('------------------task2 end---------------------');